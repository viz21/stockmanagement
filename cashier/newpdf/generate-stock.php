...
public function Header()
{
	...
	$this->Cell(0,6,"Monthly Profit report",0,1,'C');
	$this->Ln(10);
...
$pdf->Table('SELECT sum(`dailyIncome`) as NetIncome,sum(`dailyExpense`) as NetExpense,sum(`netBalance`) as NetProfit FROM `fin_dailybalance` WHERE `bDate` like "%'.$month.'%"');
$pdf->AddPage();
