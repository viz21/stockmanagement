...
$stockNameR=$_POST['stockName'];
$unitPriceR=$_POST['unitPrice'];
$discPriceR=$_POST['discPrice'];
$supplierNameR=$_POST['supplierName']; 
/*$repId=4;
$qty=10;
$stockId=36;
...
$elem_unload=count($unload_decode);
 ...
 $sql1="SELECT count(*) as cnt FROM temp_unload where stockID='".$stockId."'";
$result=mysqli_query($conn,$sql1);
$row=mysqli_fetch_assoc($result);
 if($row['cnt']==0){
  $sqla= "INSERT INTO temp_unload(stockID,productname,quntity,discount,sellingprice,supplierName) VALUES('".$stockId."','".$stockNameR."','".$qty."','".$discount."','".$unitPriceR."','".$supplierNameR."')";
    $result=mysqli_query($conn,$sqla);
 }
 else{
  $sql2="SELECT * FROM temp_unload where stockID='".$stockId."'";
  $result=mysqli_query($conn,$sql2);
  $row=mysqli_fetch_assoc($result);
  $updatedqty=(int)$row['quntity']+(int)$qty;
  echo "update qty ".$updatedqty;
