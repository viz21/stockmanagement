	...
	for($i=0;$i<$size;$i++){
		if((int)$list[$i]['stockID']==$stockId){
			if($quantity<=(int)$list[$i]['quntity']){
				...
				$sql = "INSERT INTO tempcart(stockId,stockName,quantity,unitPrice,totPrice,discPrice,repId,supplierName,status) VALUES('$stockId','$stockName','$quantity','$unitPrice','$totPrice','$discPrice',$repId,'$supplierName',1)";
				$result = mysqli_query($conn, $sql);
				$insertid=mysqli_insert_id($conn);
				if ($result) {
					$sql = "INSERT INTO tempcart_rep_lorry(id,stockId,stockName,quantity,unitPrice,totPrice,discPrice,repId,supplierName) VALUES($insertid,'$stockId','$stockName','$quantity','$unitPrice','$totPrice','$discPrice',$repId,'$supplierName')";
