...
$description="Credit Transaction from ".$retailerName;
echo $description;
...
if(strcmp($method, "Cash")==0){
    ...
    if($result2){
     ...
     $sql2 ="SELECT * from load_lorry_qty WHERE repID='$repId'";
    $result2=mysqli_query($conn,$sql2);
    $row=mysqli_fetch_array($result2);
    $LorryNo=$row['lorryNumber'];

    $sql = "INSERT INTO fin_chequedetails(chequeDate,retailerId,retailerName,Amount,chequeNumber,LorryNo) VALUES('$chequeDate','$retailerId','$retailerName','$amount','$chNum','$LorryNo')";
    $result = mysqli_query($conn, $sql);
